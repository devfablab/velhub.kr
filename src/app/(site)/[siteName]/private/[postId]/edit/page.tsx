...
export default async function Page(context: RouteContext) {
  ...
  const initialError =
    initialBoard.error || initialPost.error || (!initialPost.data?.post ? '글 수정 정보를 불러오지 못했습니다.' : '');
  ...
  return (
    <Container pageBack={`/${siteName}/private`} pageTitle="글 수정" pageFin>
      <Opt
        ...
        initialStatus={
          initialBoard.status === 401 || initialPost.status === 401
            ? 401
            : Math.max(initialBoard.status, initialPost.status)
        }
      />
    </Container>
  );
