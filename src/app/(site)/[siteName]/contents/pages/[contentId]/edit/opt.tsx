...
export default function Opt() {
  ...
  async function handleCheckSlug() {
    ...
    try {
      ...
      const response = await fetch(
        `/api/pages/check-slug?siteName=${siteName}&boardName=${normalizeBoardName(boardName)}&slug=${slug}&currentSlug=${initialSlug}`,
        {
          method: 'GET',
          credentials: 'include',
        },
  ...
  async function handleSubmit(event: FormSubmitEvent) {
    ...
    try {
      ...
      router.replace(`/${siteName}/contents/pages/${result.slug}`);
    } catch (unknownError) {
      if (unknownError instanceof Error) {
