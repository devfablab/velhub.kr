    ...
    <div className={`container ${styles.container}`}>
      <div className={`content ${styles.content} ${styles['content-payments']}`}>
        <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 1, mb: 2 }}>
          <ToggleButtonGroup exclusive value={rangeType} size="small" onChange={handleRangeTypeChange}>
          ...
          {responseData && rangeType === 'half' ? (
            ...
            >
              ...
              {responseData.filters.halves.map((halfOption) => (
                <MenuItem key={`${halfOption.year}-${halfOption.half}`} value={`${halfOption.year}-${halfOption.half}`}>
                  {halfOption.label}
                </MenuItem>
              ))}
            </Select>
          ...
          <Button type="button" variant="outlined" onClick={handleExcelDownload}>
            엑셀 다운로드
          </Button>
        </Box>
