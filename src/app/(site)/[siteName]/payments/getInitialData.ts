...
export function getRevenueSummary(siteName: string) {
  return getSiteApiData<RevenueSummaryResponse>(`/api/revenue/summary?siteName=${encodeURIComponent(siteName)}`, '수익정산 홈 정보를 불러오지 못했습니다.');
}

export function getRevenueList(siteName: string, type: RevenueListType, searchParams: Record<string, string | string[] | undefined>) {
  const query = new URLSearchParams({ siteName, page: typeof searchParams.page === 'string' ? searchParams.page : '1', pageSize: '20' });
