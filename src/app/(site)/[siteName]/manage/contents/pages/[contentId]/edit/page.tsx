...
export default async function Page({ params }: Props) {
  ...
  const statusData = await getSiteApiData<StatusResponse>(
    `/api/manage/contents/pages/status?siteName=${normalizedSiteName}`,
    '페이지 상태를 불러오지 못했습니다.',
  );
  let contentData = null;
  if (statusData.data?.boardName) {
    contentData = await getSiteApiData<ContentResponse>(
      `/api/boards/${statusData.data.boardName}/${contentId}?siteName=${normalizedSiteName}`,
      '글을 불러오지 못했습니다.',
    );
  }
  return (
    <Opt
      ...
      initialContent={contentData?.data ?? null}
