...
export default async function Page({ params }: RouteContext) {
  ...
  if (status.error || !status.data?.boardName)
    return <Opt initialData={null} initialError={status.error || '페이지 게시판을 찾을 수 없습니다.'} />;
  const content = await getSiteApiData<InitialPageDetail>(
    `/api/boards/${status.data.boardName}/${contentId}?siteName=${siteName}`,
    '페이지 정보를 불러오지 못했습니다.',
  );
