...
export default async function Page({ params }: Props) {
  ...
  const [boardData, seriesData, prefixData] = await Promise.all([
    getSiteApiData<BoardInfoResponse>(`/api/boards/${normalizedBoardName}?siteName=${normalizedSiteName}`, '게시판 정보를 불러오지 못했습니다.'),
  ...
  return <Opt initialBoard={boardData?.data} initialSeries={seriesData?.data} initialPrefix={prefixData?.data} initialError={boardData?.error || seriesData?.error || prefixData?.error} />;
}
