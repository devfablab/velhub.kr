...
export default async function Page(context: SearchContext) {
  ...
  const header = await getSiteApiData<HeaderResponse>(
    `/api/header/site?siteName=${normalizedSiteName}`,
    '권한 정보를 불러오지 못했습니다.',
  );
  const isStaff =
    header.data?.globalRole === 'admin' || header.data?.siteRole === 'owner' || header.data?.siteRole === 'manager';
  ...
  if (siteInfo.data.site_type === 'blog') {
    ...
    if (status.data?.hasBoard && status.data.boardName && !(filter && !isStaff)) {
      const board = await getSiteApiData<BoardContentsResponse>(
        `/api/boards/${status.data.boardName}?siteName=${normalizedSiteName}&page=${page}${size}${filter}`,
        '출간된 블로그 글 목록을 불러오지 못했습니다.',
      );
      ...
      initialData = {
        ...
        boards: [],
        manageContents: null,
        board: board.data?.board ?? null,
        boardName: status.data.boardName,
        posts: board.data?.contents ?? [],
        totalPage: board.data?.totalPage ?? 1,
    ...
    const boards = await getSiteApiData<BoardsResponse>(
      `/api/boards?siteName=${normalizedSiteName}&manageContents=true`,
      '게시판을 불러오지 못했습니다.',
    );
  ...
  return (
    <Container pageTitle="콘텐츠 관리" pageBack={`/${siteName}/manage`} menu="contents">
      <Opt key={`${page}:${size}:${filter}`} initialData={initialData} initialError={initialError} />
    </Container>
  );
}
