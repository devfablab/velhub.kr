...
export type WithdrawnUsersResponse = {
  ok?: boolean;
...
export default function Opt({ initialData, initialError }: OptProps) {
  ...
  const [users, setUsers] = useState<WithdrawnUserRow[]>(Array.isArray(initialData?.users) ? initialData.users : []);
  ...
  return (
    <Container pageTitle="멤버 관리" pageBack={`/${siteName}/manage`} menu="members">
                    ...
                    <TableRow key={user.userId}>
                      ...
                      <TableCell sx={{ whiteSpace: 'nowrap' }}>
                        {user.restrictionType
                          ? user.messageStatus
                            ? memberRestrictionMessageStatusLabels[user.messageStatus]
                            : '도착 전'
                          : null}
                      </TableCell>
                      <TableCell sx={{ whiteSpace: 'nowrap' }}>
      ...
      <MemberRestrictionMessageDialog
        ...
        endpoint={
          messageUser?.restrictionType
            ? `/api/manage/members/restriction-messages/${messageUser.userId}/${messageUser.restrictionType}?siteName=${siteName}`
            : ''
        }
        ownSenderType="staff"
