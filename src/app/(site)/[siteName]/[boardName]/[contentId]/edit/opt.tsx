...
function createDrawStateFromContent(content: ContentResponse['content']): DrawState {
  ...
  return {
    type: content.draw_type,
    limit: typeof content.draw_limit === 'number' && Number.isFinite(content.draw_limit) ? content.draw_limit : 1,
    endsAt: content.draw_ends_at ? parseDateTimeValue(content.draw_ends_at) : null,
  };
}
...
export default function Opt({
  ...
  const [galleryMinimumWarningOpen, setGalleryMinimumWarningOpen] = useState(false);
  ...
  function applyGalleryDialogImages() {
    ...
    if (totalCount < 2) {
      setGalleryMinimumWarningOpen(true);
      return;
    }
    ...
    <Container pageBack={`/${siteName}/${boardName}/${contentId}`} pageTitle="글 수정" pageFin>
      <div className="container">
        ...
        <div className={`${styles.content} content`}>
          ...
          <PopupMessage
            open={galleryMinimumWarningOpen}
            message="2개 이상의 이미지를 필수로 등록하셔야 합니다."
            kind="error"
            onClose={() => setGalleryMinimumWarningOpen(false)}
          />
          ...
          {isMobile ? (
            ...
            >
              ...
              <DialogActions>
                ...
                <button type="button" onClick={applyGalleryDialogImages}>
                  이미지 업로드
                </button>
              </DialogActions>
            </Dialog>
