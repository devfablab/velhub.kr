...
export async function POST(request: Request, context: RouteContext) {
  try {
    ...
    const categoryCount = await supabaseAdmin
      .from('board_categories')
      .select('id', { count: 'exact', head: true })
      .eq('site_id', rhizome.data.id)
      .eq('board_id', board.data.id);

    if (categoryCount.error) {
      return Response.json({ error: '카테고리 개수를 확인하지 못했습니다.' }, { status: 500 });
    }

    if ((categoryCount.count ?? 0) >= 30) {
      return Response.json({ error: '카테고리는 게시판당 최대 30개까지 등록할 수 있습니다.' }, { status: 400 });
    }
    ...
    const nextSortOrder =
      typeof lastCategory.data?.sort_order === 'number' ? Number(lastCategory.data.sort_order) + 1 : 1;
    ...
    if (insertCategory.error || !insertCategory.data) {
      return Response.json({ error: '카테고리 등록에 실패했습니다.' }, { status: 500 });
    }
    ...
    if (unknownError instanceof Error) {
      return Response.json({ error: unknownError.message || '카테고리 등록에 실패했습니다.' }, { status: 500 });
    }
