...
export async function DELETE(request: Request, context: RouteContext) {
  try {
    ...
    const category = await supabaseAdmin
      ...
      .select('id, category_key, board_id, site_id')
      .eq('site_id', rhizome.data.id)
    ...
    const connectedPost = await supabaseAdmin
      .from('posts')
      .select('id')
      .eq('board_id', board.data.id)
      .contains('categories', [category.data.id])
      .limit(1)
      .maybeSingle();

    if (connectedPost.error) {
      return Response.json({ error: '카테고리 연결 글을 확인하지 못했습니다.' }, { status: 500 });
    }

    if (connectedPost.data) {
      return Response.json({ error: '연결된 글이 있는 카테고리는 삭제할 수 없습니다.' }, { status: 400 });
    }

    const deleteCategory = await supabaseAdmin.from('board_categories').delete().eq('id', category.data.id);
    ...
    if (unknownError instanceof Error) {
      return Response.json({ error: unknownError.message || '카테고리 삭제에 실패했습니다.' }, { status: 500 });
    }
