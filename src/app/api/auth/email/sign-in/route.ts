  ...
  const sendResult = await getResendClient().emails.send({
    ...
    html: `
      <table style="border-collapse:collapse;width:100%;border-style:none;margin-left:auto;margin-right:auto" border="0">
        ...
        <tr><td><div style="max-width:575px;width:100%;padding:23px;margin:0 auto;box-sizing:border-box;font-family:'Apple SD Gothic Neo', 'Noto Sans KR','Malgun Gothic', '맑은 고딕', sans-serif;color:#181818;"><h1>데브허브 비밀번호 재설정</h1><p>콘텐츠에 가치를 더하는 복합 허브 서비스, 데브허브입니다.</p><p>계정 보호를 위해 비밀번호를 초기화했습니다.</p><p>로그인 시도가 여러 차례 실패해 기존 비밀번호는 더 이상 사용할 수 없습니다.</p><p>아래 버튼을 눌러 새 비밀번호를 설정해 주세요.</p><p style="text-align:center"><a href="${generateLinkResult.data.properties.action_link}" style="background-color:#eeb400;color:#181818;display:inline-block;padding:12px 23px;border-radius:12px;font-weight:bolder;text-decoration:none">비밀번호 재설정</a></p><p>본인이 시도하지 않았다면 비밀번호를 변경한 뒤 데브허브 고객지원으로 문의해 주세요.</p><p><strong style="font-size:12px">Everyday, Everywhere, Everymoments - Velhub</strong></p></div></td></tr>
...
async function sendCaptchaRequiredEmail(email: string) {
  const sendResult = await getResendClient().emails.send({
    from: getMailFrom(),
    to: email,
    subject: '[데브허브] 로그인 보안 안내',
    html: `
      <table style="border-collapse:collapse;width:100%;border-style:none;margin-left:auto;margin-right:auto" border="0">
        <tr><td style="background-color:#181818"><div style="max-width:575px;width:100%;padding:23px;box-sizing:border-box;margin:0 auto"><img style="border-style:none" src="https://velhub.xyz/velhub-1-webmail.png" alt="데브허브" width="106" height="24"></div></td></tr>
        <tr><td><div style="max-width:575px;width:100%;padding:23px;margin:0 auto;box-sizing:border-box;font-family:'Apple SD Gothic Neo', 'Noto Sans KR','Malgun Gothic', '맑은 고딕', sans-serif;color:#181818;"><h2>로그인 보안 안내</h2><p>콘텐츠에 가치를 더하는 복합 허브 서비스, 데브허브입니다.</p><p>회원님의 계정에서 로그인에 5회 실패했습니다.</p><p>계정 보호를 위해 다음 로그인부터 추가 확인이 필요합니다.</p><p>본인이 시도하지 않았다면 비밀번호를 변경해 주세요.</p><p><strong style="font-size:12px">Everyday, Everywhere, Everymoments - Velhub</strong></p></div></td></tr>
        <tr><td style="background-color:#181818"><div style="max-width:575px;width:100%;padding:23px;margin:0 auto;box-sizing:border-box;font-family:'Apple SD Gothic Neo', 'Noto Sans KR','Malgun Gothic', '맑은 고딕', sans-serif;"><span style="color:#d7d7d7;font-size:12px">&copy; <img src="https://velhub.xyz/velhub-2-webmail.png" alt="데브런닷스튜디오" width="90" height="12"> All rights reserved. <strong style="color:#ff69b4;padding-left:12px">&hearts; velhub</strong></span></div></td></tr>
      </table>
    `,
  });
  ...
  if (sendResult.error) {
    throw new Error(sendResult.error.message || '로그인 보안 안내 메일을 보내지 못했습니다.');
  }
}
  ...
  try {
    ...
    if (signInResult.error) {
      ...
      if (nextFailureCount === 5) {
        try {
          await sendCaptchaRequiredEmail(email);
        } catch (emailError) {
