...
export async function POST(request: Request, context: RouteContext) {
  try {
    ...
    const general = await supabaseAdmin
      .from('member_general')
      .select('id')
    ...
    if (general.error || !general.data) {
      return Response.json({ error: '기본 정보를 먼저 등록해주세요.' }, { status: 400 });
    }

    const tableName = getTableName(itemType);

    const maxSortOrder = await supabaseAdmin
      .from(tableName)
      .select('sort_order')
      .eq('site_id', rhizome.data.id)
      .eq('member_id', session.rhizomeStigmaId)
    ...
    if (maxSortOrder.error) {
      return Response.json({ error: '정렬 순서를 확인하지 못했습니다.' }, { status: 500 });
    }

    const nextSortOrder = Number(maxSortOrder.data?.sort_order ?? 0) + 1;

    const insertItem = await supabaseAdmin
      .from(tableName)
      .insert({
        ...built.payload,
    ...
    if (insertItem.error || !insertItem.data) {
      return Response.json({ error: '항목 등록에 실패했습니다.' }, { status: 500 });
    }
    ...
    if (unknownError instanceof Error) {
      return Response.json({ error: unknownError.message || '항목 등록에 실패했습니다.' }, { status: 500 });
    }
