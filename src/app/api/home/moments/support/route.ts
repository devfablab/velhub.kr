...
export async function GET(request: Request) {
  ...
  try {
    ...
    const siteIds = selections
      .filter((s) => s.selector_type === 'creator_site')
      .map((s) => s.target_id);
    ...
    const [sitesResult, postsResult] = await Promise.all([
      siteIds.length > 0
        ? supabaseAdmin.from('rhizomes').select('site_key, site_label, summary, site_type, profile_picture, promotion_image, created_at, post_count:posts(count)').in('id', siteIds)
        : { data: [], error: null },
      postIds.length > 0
    ...
    const sites = (sitesResult.data || []).map((site: any) => ({
