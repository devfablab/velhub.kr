...
async function sendBlockEmail({
  ...
  await getResendClient().emails.send({
    from: getMailFrom(),
    to: email,
    subject: `[데브허브] ${siteLabel} 활동정지 안내`,
    ...
    if (typeof emailResult.data?.email === 'string' && emailResult.data.email.trim()) {
      try {
        await sendBlockEmail({
          ...
          siteLabel: siteResult.data?.site_label ?? siteName,
          reason,
          blockedAt: new Date().toISOString(),
          releaseAt: parsedBlockTerm?.toISOString() ?? null,
        });
      } catch (emailError) {
        console.error('[users/block] email error', emailError);
      }
    }
