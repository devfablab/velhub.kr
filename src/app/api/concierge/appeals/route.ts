...
export async function GET(request: Request) {
  try {
    ...
    const items = await loadAppealCenterItems({
    ...
    console.error('[concierge/appeals] unexpected error', unknownError);

    if (unknownError instanceof Error) {
      return Response.json({ error: unknownError.message || '소명 내역을 불러오지 못했습니다.' }, { status: 500 });
    }

    return Response.json({ error: '소명 내역을 불러오지 못했습니다.' }, { status: 500 });
  }
}
