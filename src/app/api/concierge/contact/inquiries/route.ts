...
async function getPaymentOptions(stigmaId: string, cancellationOnly = true) {
  ...
  let paymentQuery = db
    .from('payments')
    .select(
      'id, order_no, payment_type, target_type, target_id, amount, currency, approved_at, status, refunded_amount, guardian_identity_verified, provider, payment_method, created_at, raw_data',
    )
...
export async function POST(request: NextRequest) {
  ...
  if ((isPaymentProblem || inquiryType === 'minor_purchase_cancellation') && paymentId) {
    ...
    const paymentOptions = await getPaymentOptions(currentStigma.stigmaId, false);
    resolvedPaymentLabel = paymentOptions.find((option) => option.id === paymentId)?.label ?? null;
  }
  ...
  if (paymentId) {
    const { error: orderError } = await db
      .from('inquiry_orders')
      .insert({ inquiry_id: inquiry.id, payment_id: paymentId, payment_label: resolvedPaymentLabel });
    if (orderError) {
      await db.from('inquiries').delete().eq('id', inquiry.id);
