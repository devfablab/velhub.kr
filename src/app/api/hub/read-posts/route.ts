...
export async function GET(request: Request) {
  try {
    ...
    if (readsResult.error) {
      return Response.json({ error: '읽은 글 목록을 불러오지 못했습니다.' }, { status: 500 });
    }

    const readRows = (readsResult.data ?? []) as ReadRow[];
    const siteIds = Array.from(new Set(readRows.map((row) => row.site_id)));
    ...
    const resultPosts = filteredReads
      .map((read) => {
        const site = siteMap.get(read.site_id);
        const post = postMap.get(read.post_id);
        const boardKey = boardMap.get(read.board_id);
    ...
    if (unknownError instanceof Error) {
      return Response.json({ error: unknownError.message || '읽은 글 목록을 불러오지 못했습니다.' }, { status: 500 });
    }
