...
export async function GET(_request: Request, context: RouteContext) {
  try {
    ...
    if (communityResult.error) {
      return Response.json({ error: 'communities 정보를 불러오지 못했습니다.' }, { status: 500 });
    }

    if (!communityResult.data) {
      return Response.json({ error: 'communities 정보를 찾을 수 없습니다.' }, { status: 404 });
    }
    ...
    if (unknownError instanceof Error) {
      return Response.json(
        { error: unknownError.message || 'communities 정보를 불러오지 못했습니다.' },
        { status: 500 },
      );
    }
