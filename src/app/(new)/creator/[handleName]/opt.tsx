...
function ProfileForm({ profile, onCancel, onSaved }: { profile: CreatorProfile; onCancel: () => void; onSaved: (profile: CreatorProfile) => void }) {
  ...
  const [links, setLinks] = useState<CreatorLink[]>(profile.links);
  ...
  const save = async () => {
    ...
    try {
      ...
      onSaved({ ...payload.creator, links: links.map((link) => ({ ...link, url: withProtocol(link.url) })) });
    } catch (error) {
...
export default function Opt({ handleName }: { handleName: string }) {
  ...
  useEffect(() => { load(page).catch((error) => setMessage(error instanceof Error ? error.message : '작가 정보를 불러오지 못했습니다.')); }, [load, page]);
